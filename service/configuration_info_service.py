...
from util.config import write_to_file, get_config_value, set_config_value, get_material_type_map, get_storage_temp_map, get_type_to_collection_map, reload_all_maps, get_miabis_on_fhir, get_miabis_material_type_map, get_miabis_storage_temp_map
...
def __create_config_snapshot_directory(timestamp, file_type):
    ...
    snapshot_dir_name = f"config_{timestamp}_{file_type}"
...
def __list_directories(request):
    ...
    """
    try:
        base_dir = os.environ.get('DIR_PATH', '/')
        ...
        try:
            ...
            return make_response(jsonify({'error': f'Error listing directory: {str(e)}'}), 500)
...
def __validate_folder_path_request(content: dict) -> tuple[str, dict | None]:
    ...
    if '..' in folder_path or folder_path.startswith('..'):
        return folder_path, make_response(jsonify({
            'success': False,
...
def __find_supported_data_files(folder_path: str) -> tuple[list[dict], dict | None]:
        ...
        for file_name in files:
            ...
            if os.path.isfile(file_path):
                ...
                if ext in supported_extensions:
                    data_files.append({'path': file_path, 'ext': ext, 'name': file_name})
    except PermissionError:
        return [], make_response(jsonify({
            'success': False,
    ...
    if not data_files:
        return [], make_response(jsonify({
            'success': False,
            'message': f'No supported data files found in {folder_path}. Supported formats: JSON, CSV, XML'
        }), 404)
...
def __read_first_data_file(data_files: list[dict]) -> tuple[str, str, str, dict | None]:
    ...
    try:
        ...
        return '', file_name, file_ext, make_response(jsonify({
            'success': False,
            'message': f'Error reading {file_name}: {str(read_error)}'
        }), 500)
...
def __run_data_parsing_validation(validate_all_files: bool, errors: dict[str, list[str]], miabis_on_fhir: bool = False) -> bool:
    ...
    try:
        ...
        condition_service = ConditionService(repository_factory.create_condition_repository(miabis_on_fhir))
